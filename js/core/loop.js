(function initLoop(window, OG) {
  ...
  function initAudio() { return OG.audio.initAudio(); }
  function makeGain(vol) { return OG.audio.makeGain(vol); }
  function getMinSoundIntervalMs() { return OG.audio.getMinSoundIntervalMs(); }
  function shouldThrottleAudio(allowBypassWindow = false) { return OG.audio.shouldThrottleAudio(allowBypassWindow); }

  function playTone(freq, type, vol, attack, decay, startTime) { return OG.audio.playTone(freq, type, vol, attack, decay, startTime); }
  function playNoiseBurst(vol, decay, startTime, filterType = 'bandpass', filterFreq = 1100, q = 0.8) { return OG.audio.playNoiseBurst(vol, decay, startTime, filterType, filterFreq, q); }
  function startBossDrone() { return OG.audio.startBossDrone(); }
  function stopBossDrone() { return OG.audio.stopBossDrone(); }
  function escalateBossDrone() { return OG.audio.escalateBossDrone(); }
  function startLastLifeDrone() { return OG.audio.startLastLifeDrone(); }
  function stopLastLifeDrone() { return OG.audio.stopLastLifeDrone(); }
  function soundOk() { return OG.audio.soundOk(); }
  function soundGood(multiplier) { return OG.audio.soundGood(multiplier); }
  function soundPerfect(multiplier) { return OG.audio.soundPerfect(multiplier); }
  function soundCornerBonus(worldNum = 1) { return OG.audio.soundCornerBonus(worldNum); }
  function soundMultiplierUp(multiplier) { return OG.audio.soundMultiplierUp(multiplier); }
  function soundFail() { return OG.audio.soundFail(); }
  function soundLifeLost() { return OG.audio.soundLifeLost(); }
  function soundWaveClear() { return OG.audio.soundWaveClear(); }
  function soundWorldClear() { return OG.audio.soundWorldClear(); }
  function soundShieldBreak() { return OG.audio.soundShieldBreak(); }
  function soundBossShieldHit(hp) { return OG.audio.soundBossShieldHit(hp); }
  function soundCoreExposed() { return OG.audio.soundCoreExposed(); }
  function soundCoreDamage() { return OG.audio.soundCoreDamage(); }
  function soundBossDefeated() { return OG.audio.soundBossDefeated(); }
  function soundLifeZone() { return OG.audio.soundLifeZone(); }
  function soundLifeGained() { return OG.audio.soundLifeGained(); }
  function soundUIClick() { return OG.audio.soundUIClick(); }
  function playPop(multiplier, isPerfect, isFail = false) { return OG.audio.playPop(multiplier, isPerfect, isFail); }

  function loadAudioFile(url) { return OG.audio.loadAudioFile(url); }
  function getBaseTrackForLevel(levelIdx) { return OG.audio.getBaseTrackForLevel(levelIdx); }
  function startDynamicMusic(baseTrackPath) { return OG.audio.startDynamicMusic(baseTrackPath); }
  function disposeMusicNodes() { return OG.audio.disposeMusicNodes(); }
  function stopDynamicMusic() { return OG.audio.stopDynamicMusic(); }
  function baseVolumeForMultiplier(currentMultiplier) { return OG.audio.baseVolumeForMultiplier(currentMultiplier); }
  function ensureCorrectMusicForLevel() { return OG.audio.ensureCorrectMusicForLevel(); }
  function updateMusicState(currentMultiplier, isBossActive) { return OG.audio.updateMusicState(currentMultiplier, isBossActive); }
  ...
  function vibrate(pattern) { return OG.audio.vibrate(pattern); }
  ...
  const stateBridge = OG.state.legacy = OG.state.legacy || {};
  ...
  function computeWorldPalette(level) {
    ...
    if (level && level.boss === 'phoenix') {
      let phase = 0;
      if (OG.systems && OG.systems.phoenixBoss && typeof OG.systems.phoenixBoss.getPhaseIdx === 'function') {
        phase = OG.systems.phoenixBoss.getPhaseIdx();
      }
  ...
  function getPointOnShape(t, shape, cx, cy, radius) {
    ...
    return OG.systems.rendering.getPointOnShape(adjustedAngle, shape, cx, cy, radius);
  }

  function buildShapePath(ctx, shape, cx, cy, radius, startAngle, endAngle, steps = 40) {
    const offset = getArenaAngleOffset();
    return OG.systems.rendering.buildShapePath(
      ctx,
      shape,
      cx,
      cy,
  ...
  function toggleShop(show) { return OG.ui.shop.toggleShop(show); }
  function toggleSettings(show) { return OG.ui.settings.toggleSettings(show); }
  function applySettingsUI() { return OG.ui.settings.applySettingsUI(); }
  function toggleMusicSetting() { return OG.ui.settings.toggleMusicSetting(); }
  function toggleSfxSetting() { return OG.ui.settings.toggleSfxSetting(); }
  function toggleHapticsSetting() { return OG.ui.settings.toggleHapticsSetting(); }
  function updateShopUI() { return OG.ui.shop.updateShopUI(); }
  function updateWorkshopUI() { return OG.ui.shop.updateWorkshopUI(); }
  function buyItem(id, cost) { return OG.ui.shop.buyItem(id, cost); }
  function equipSkin(id) { return OG.ui.shop.equipSkin(id); }
  ...
  function renderShopOrbPreview(previewEl, skinId) {
    ...
    const reg = OG.entities && OG.entities.spheres && OG.entities.spheres.registry;
    const meta = reg ? reg[skinId] : null;
    const rColor = (meta && RARITY_COLOR[meta.rarity]) ? RARITY_COLOR[meta.rarity] : '#aaaaaa';

    const sr = OG.entities.spheres.runtime;
  ...
  function getParticle() { return OG.entities.particles.getParticle(); }
  function releaseParticle(particle) { return OG.entities.particles.releaseParticle(particle); }
  function getPopup() { return OG.entities.effects.getPopup(); }
  function releasePopup(popup) { return OG.entities.effects.releasePopup(popup); }
  ...
  function resetRunState() {
    ...
    const _sr = OG.entities.spheres.runtime;
  ...
  function updateLastLifeState() {
    ...
    if (isLastLife && !wasLastLife) {
      ...
      if (OG.audio.audioCtx) startLastLifeDrone();
  ...
  function loseLife(reason) {
    if (typeof OG !== 'undefined' && OG.debug && OG.debug.infiniteLives) {
  ...
  function createParticles(x, y, color, count = 20) { return OG.entities.particles.createParticles(x, y, color, count); }
  function createUpwardBurstParticles(x, y, color, count = 36) { return OG.entities.particles.createUpwardBurstParticles(x, y, color, count); }
  ...
  function createPopup(x, y, text, color, hitQuality = null) { return OG.entities.effects.createPopup(x, y, text, color, hitQuality); }
  function showComboPopup(multiplierLevel) { return OG.entities.effects.showComboPopup(multiplierLevel); }
  function showNearMissReplay(reason, nearestEdgeDistance) { return OG.entities.effects.showNearMissReplay(reason, nearestEdgeDistance); }
  function showSurvivalNearMissShock(nearestEdgeDistance) { return OG.entities.effects.showSurvivalNearMissShock(nearestEdgeDistance); }
  function triggerIntensity(level) { return OG.entities.effects.triggerIntensity(level); }
  function clearIntensity() { return OG.entities.effects.clearIntensity(); }
  function createShockwave(color, speed = 40) { return OG.entities.effects.createShockwave(color, speed); }
  function createTargetHitRipple(x, y, color = '#ffffff') { return OG.entities.effects.createTargetHitRipple(x, y, color); }
  function triggerTargetHitFeedback(target, x, y) { return OG.entities.effects.triggerTargetHitFeedback(target, x, y); }
  function triggerScreenShake(intensity = 5) {
    if (levelData && levelData.id === '2-6' && levelData.boss === 'prism') return;
    return OG.entities.effects.triggerScreenShake(intensity);
  }
  function pulseBrightness(amount = 1.6, duration = 120) { return OG.entities.effects.pulseBrightness(amount, duration); }
  function scheduleBossSpawn(delay = 700) { return OG.entities.boss.scheduleBossSpawn(delay); }
  function pauseGameplayBriefly(duration = 750) { return OG.entities.boss.pauseGameplayBriefly(duration); }
  ...
  function normalizeAngle(a) { return OG.systems.collision.normalizeAngle(a); }

  function signedAngularDistance(from, to) { return OG.systems.collision.signedAngularDistance(from, to); }

  function getTargetApproachIntensity(target, playerAngle, playerDirection) {
    return OG.systems.collision.getTargetApproachIntensity(target, playerAngle, playerDirection);
  }
  ...
  function generateShareCard() { return OG.ui.share.generateShareCard(); }
  function downloadCard(card) { return OG.ui.share.downloadCard(card); }

  function updateMultiplierUI() { return OG.systems.scoring.updateMultiplierUI(); }

  function getCheckpointIndex() { return OG.systems.progression.getCheckpointIndex(); }

  function updateWaveUI() { return OG.systems.progression.updateWaveUI(); }

  function triggerBossIntro() { return OG.entities.boss.triggerBossIntro(); }

  function playBossCinematic() { return OG.entities.boss.playBossCinematic(); }
  ...
  function loadLevel(idx) {
    ...
    if (OG.systems && OG.systems.tutorial) {
       OG.systems.tutorial.handleLevelStart(levelData.id);
    }
    if (OG.systems && OG.systems.progression && typeof OG.systems.progression.markStageStartRunStats === 'function') {
      OG.systems.progression.markStageStartRunStats();
    }
    ...
    window.ghostShieldActive = !!(OG.entities.spheres.runtime &&
      OG.entities.spheres.runtime.getCombinedValue('ghostSave'));
      ...
      if (audioCtx) {
        ...
        if (!_mb) {
          ...
          if (typeof OG !== 'undefined' && OG.audio) {
            OG.audio.currentMusicState = { mult: -1, boss: null };
          }
  ...
  function buildTarget(start, size, config = {}) { return OG.entities.target.buildTarget(start, size, config); }

  function spawnWorld2CornerBonusTargets() { return OG.entities.target.spawnWorld2CornerBonusTargets(); }

  function buildCornerPrecisionTarget(anchorAngle, options = {}) { return OG.entities.target.buildCornerPrecisionTarget(anchorAngle, options); }

  function buildDualTarget(startAngle, options = {}) { return OG.entities.target.buildDualTarget(startAngle, options); }

  function buildSplitTarget(startAngle, size, options = {}) { return OG.entities.target.buildSplitTarget(startAngle, size, options); }

  function spawnWorld2MechanicTargets() { return OG.entities.target.spawnWorld2MechanicTargets(); }

  function spawnTargets() { return OG.systems.spawning.spawnTargets(); }
  ...
  function draw() {
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      const targetRenderer = OG.entities && OG.entities.targetRenderers;
  ...
  function isInsideTarget(playerAngle, t) { return OG.entities.target.isInsideTarget(playerAngle, t); }

  function showTempText(text, color, duration) { return OG.entities.effects.showTempText(text, color, duration); }
  ...
  function update() {
    ...
    if (levelData && levelData.boss === 'phoenix') {
      if (OG.systems && OG.systems.phoenixBossV2 && OG.systems.phoenixBossV2.isActive()) {
        OG.systems.phoenixBossV2.tick(frameNow);
      } else if (OG.systems && OG.systems.phoenixBoss && OG.systems.phoenixBoss.isActive()) {
        OG.systems.phoenixBoss.tick(frameNow);
      }
    }
    // ─────────────────────────────────────────────────────────────────────────
  ...
  function handleFail(reason, failEdgeDistance = Infinity) {
    ...
    if (lives <= 0) {
      ...
      if (OG.ui.overlay && OG.ui.overlay.animateScore) {
        OG.ui.overlay.animateScore(ui.runScoreDisplay, score);

        const totalHits = (runPerfectCount || 0) + (runGoodCount || 0) + (runOkCount || 0);
        OG.ui.overlay.showRunGrade(runPerfectCount || 0, totalHits);

        // On fail, we show minimal XP progress (maybe 2 XP for the effort?)
        OG.ui.overlay.showXPEarned(2);
      }
  ...
  function attemptCoinRevive() {
    return OG.ui.overlay.attemptCoinRevive();
  }
  ...
  function tap() {
    ...
    const hitProfiles = OG.entities && OG.entities.targetHitProfiles;
    ...
    if (hitIndex !== -1) {
      ...
      const _hitRadiusBonus = OG.entities.spheres.runtime ? OG.entities.spheres.runtime.getCombinedValue('hitRadiusBonus') : 0;
      ...
      if (t.phoenixTarget && levelData.boss === 'phoenix') {
        const _v2Active = OG.systems && OG.systems.phoenixBossV2 && OG.systems.phoenixBossV2.isActive();
        const _v1Active = OG.systems && OG.systems.phoenixBoss && OG.systems.phoenixBoss.isActive();
        if (_v2Active || _v1Active) {
          const _bossMod = _v2Active ? OG.systems.phoenixBossV2 : OG.systems.phoenixBoss;
      ...
      const targetBehaviours = OG.entities && OG.entities.targetBehaviours;
      ...
      if (hitTimingTier === 'filthy-perfect') {
        ...
        if (OG.entities.spheres.runtime && OG.entities.spheres.runtime.getCombinedValue('comboPerfectBonus')) {
            comboCount++;
            streak++;
        }
        ...
        if (OG.entities.spheres.runtime && OG.entities.spheres.runtime.getCombinedValue('comboPerfectBonus')) {
            comboCount++;
            streak++;
            if (OG.ui.hud && OG.ui.hud.checkStreakMilestones) {
              OG.ui.hud.checkStreakMilestones(streak);
            }
        }
        soundMultiplierUp(multiplier);
      ...
      const _coinBonus = OG.entities.spheres.runtime ? OG.entities.spheres.runtime.getCombinedValue('coinMultiplierBonus') : 0;
    ...
    if (isPremium) centsEarned *= 2; // Premium x2 coins
      ...
      if (shouldForceHudFlush) {
        ...
        const _flawlessBonus = OG.entities.spheres.runtime ? OG.entities.spheres.runtime.getCombinedValue('flawlessBonus') : 0;
      ...
      if (isNearMiss) {
        ...
        if (isSurvivalNearMissShock) {
          ...
          if (OG.entities.spheres.runtime && OG.entities.spheres.runtime.getCombinedValue('nearMissCoin')) {
            runCents += 1;
  ...
  function triggerStageClear() { return OG.systems.progression.triggerStageClear(); }

  function startCampaign() { return OG.ui.menus.startCampaign(); }
  ...
  function returnToMenu() {
    ...
    if (
      OG.ui &&
      OG.ui.menus &&
      typeof OG.ui.menus.refreshMenuWorldPreview === 'function'
    ) {
      OG.ui.menus.refreshMenuWorldPreview();
    }
    ...
    setTimeout(function() {
      if (OG.systems && OG.systems.ceremony) {
  ...
  function changeWorld(dir) { return OG.ui.menus.changeWorld(dir); }

  function updateWorldSelectorUI() { return OG.ui.menus.updateWorldSelectorUI(); }
    ...
    worldClearDelayTimeout = setTimeout(() => {
      ...
      worldClearTallyInterval = setInterval(() => {
        ...
        if (currentDisplayCoins >= coinsEarned) {
          ...
          if (OG.ui.overlay && OG.ui.overlay.animateScore) {
            OG.ui.overlay.animateScore(ui.clearScoreDisplay, score);
            ...
            OG.ui.overlay.showXPEarned(totalXP);
          }
