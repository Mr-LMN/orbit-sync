...
let rawCoins = 0; // Tracks decimals for the 0.1 ratio
...
function triggerScreenShake() {
  ...
  setTimeout(() => canvas.style.transform = `translate(${Math.random()*10-5}px, ${Math.random()*10-5}px)`, 50);
...
function spawnTargets() {
  ...
  // MAKING SPLIT ZONES SMALLER: If more than 1 target, shrink them by 40%
...
function update() {
  ...
  // Trail Logic (Longer trail = higher multiplier)
  const x = center + Math.cos(angle) * orbitRadius; const y = center + Math.sin(angle) * orbitRadius;
  trail.push({x: x, y: y});
...
function handleFail(reason) {
  ...
  if (lives <= 0) {
    ...
    ui.subtitle.innerText = `Failed on ${levelData.title}`; ui.btn.innerText = "Try Again";
  }
}
...
function tap() {
  ...
  for(let i = 0; i < targets.length; i++) {
    ...
    if (isHit) {
      ...
      if (dist < targets[i].size / 6) hitQuality = "perfect"; // Dead center
      else if (dist < targets[i].size / 3) hitQuality = "good"; // Mid zone
      else hitQuality = "ok"; // Edge
      break;
    }
  }
  ...
  if (hitIndex !== -1) {
    let t = targets[hitIndex]; t.active = false; direction *= -1; distanceTraveled = 0; 
    
    if (hitQuality === "perfect" && currentLevelIdx >= 2) {
      multiplier = Math.min(multiplier + 1, 8); // Cap at 8x
      score += (3 * multiplier);
      createPopup(hitX, hitY - 20, "PERFECT!", multiColors[multiplier-1]);
    } else if (hitQuality === "good") {
      score += (2 * multiplier);
      createPopup(hitX, hitY - 20, "GOOD", "#fff");
    } else {
      multiplier = 1; score += 1;
      createPopup(hitX, hitY - 20, "OK", "#aaa");
    }
    
    // ECONOMY BALANCE: 10% of score earned becomes coins. Floor it for display.
    rawCoins += ((hitQuality === "perfect" ? 3 : hitQuality === "good" ? 2 : 1) * multiplier) * 0.1;
    
    ui.score.innerText = score; 
    ui.coins.innerText = Math.floor(rawCoins); 
...
function startGame() {
  ...
  if (lives <= 0) { currentLevelIdx = 0; score = 0; rawCoins = 0; ui.score.innerText = 0; ui.coins.innerText = 0; }
