...
// Hook up the Main Menu button directly
...
function updateShopUI() {
  ...
  items.forEach(id => {
    ...
    if (activeSkin === id) {
      ...
      btn.className = 'buy-btn btn-owned';
      btn.innerText = 'Equip';
...
function draw() {
  ...
  ctx.beginPath();
  ctx.arc(centerObj.x, centerObj.y, orbitRadius * 0.15, 0, Math.PI * 2);
  ...
  targets.forEach(t => {
    if (t.active) {
      ...
      if (levelData.boss && !isBossPhaseTwo) {
        ...
        ctx.beginPath();
        ctx.arc(centerObj.x, centerObj.y, orbitRadius, tCenter - t.size / 4, tCenter + t.size / 4);
        ctx.strokeStyle = t.color;
        ctx.globalAlpha = 0.8;
        ctx.stroke();
        ...
        if (currentLevelIdx >= 2 || levelData.boss) {
          ctx.beginPath();
          ctx.arc(centerObj.x, centerObj.y, orbitRadius, tCenter - t.size / 12, tCenter + t.size / 12);
  ...
  if (!inMenu) {
    trail.forEach((p, index) => {
      ...
      ctx.beginPath();
      ctx.arc(p.x, p.y, 8 * (index / trail.length), 0, Math.PI * 2);
...
function update() {
  ...
  if (!inMenu && currentLevelIdx >= 3) {
    if (distanceTraveled >= Math.PI * 2 && multiplier > 1) {
      ...
      document.getElementById('multiplierBox').style.color = "#fff";
    }
...
function handleFail(reason) {
  ...
  canvas.style.boxShadow = `inset 0 0 50px #ff3366`;
  ...
  if (lives <= 0) {
    ...
    // Fix: Make sure button Restarts the World on Fail
    ui.btn.innerText = `Restart World ${levelData.id.split('-')[0]}`;
    ui.btn.onclick = restartFromCheckpoint;
...
function tap() {
  ...
  const hitX = centerObj.x + Math.cos(angle) * orbitRadius;
  const hitY = centerObj.y + Math.sin(angle) * orbitRadius;
  ...
  if (hitIndex !== -1) {
    ...
    if (levelData.boss && isBossPhaseTwo) {
      if (hitQuality === "perfect") {
        ...
        multiplier = 1;
        ui.multiplier.innerText = multiplier;
        createPopup(centerObj.x, centerObj.y - 50, "REGENERATING...", "#ff3366");
        triggerScreenShake(10);
        isBossPhaseTwo = false;
        setTimeout(spawnTargets, 500);
        return;
    ...
    if (targets.every(tgt => !tgt.active) || stageHits >= levelData.hitsNeeded) {
      ...
      if (stageHits >= levelData.hitsNeeded) {
        ...
        if (newWorld > maxWorldUnlocked) {
          maxWorldUnlocked = newWorld;
          saveData();
        }
        ...
        ui.subtitle.innerText = `Coins Earned: ${coinsToBank}`;
        
        // Fix: Explicitly load next stage logic on the button
